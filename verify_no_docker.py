...
# ----------------------------
# Host path for Docker-free testing
# ----------------------------
DATA_DIR = "/tmp/securedrop_test/shared"

def verify(user, pwd, data_dir=DATA_DIR):
    passwd_file = os.path.join(data_dir, "passwd.json")

    # Ensure passwd.json exists
    ...
    while attempts > 0:
        ...
        if inp_usr in data["Users"]:
            ...
            if password.verify_password(stored_hash, inp_pwd):
                print(f"[INFO] Login successful: {inp_usr}")
                return inp_usr
            else:
                attempts -= 1
                print(f"[WARN] Incorrect password. Attempts remaining: {attempts}")
        else:
            attempts -= 1
            print(f"[WARN] Username not found. Attempts remaining: {attempts}")
