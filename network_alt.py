...
def get_bridge_ip():
    ...
    try:
        # connect to an arbitrary IP; won't actually send packets
...
def listen_for_users():
    ...
    sock.bind(("0.0.0.0", NETWORK_PORT))  # listen on all interfaces
...
def announce_presence(username):
    ...
    while True:
        # Read the latest discovered users
        targets = {}
        ...
        for user, ip in targets.items():
            if user != username:  # don't announce to self
                try:
...
def print_discovered_loop():
    while True:
        if os.path.exists(DISCOVERY_FILE):
            try:
                with open(DISCOVERY_FILE, "r") as f:
                    data = json.load(f)
                    print(f"[INFO] Discovered users: {data}")
            except json.JSONDecodeError:
                pass
        time.sleep(5)
